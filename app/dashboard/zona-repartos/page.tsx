...
function ZonasRepartos() {
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      if (error.response && error.response.status === 409) { // Suponiendo que el código de error es 409 (Conflict)
        toast.error("Una o más comunas o regiones ya tienen asignado un valor de despacho.");
      } else {
        console.error(
          "Error al",
  ...
  {zonas.map((zone) => (
    <tr key={zone.id}>
      ...
      <td className="px-6 py-4 md:whitespace-nowrap">
        <div className="text-sm text-gray-900">${zone.amount.toLocaleString("es-CL")}</div>
      </td>
        ...
        >
          ...
          <div className="mt-4 flex gap-2">
            ...
            <button
              ...
              className="shadow bg-red-500 hover:bg-red-700 uppercase text-white font-bold py-2 px-4  flex-wrap"
              style={{ borderRadius: "var(--radius)" }}
            >
