// components/ContentBlockForm.js
'use client'
...
const ContentBlockForm = () => {
  ...
  const handleContentSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL_BO_CLIENTE}/api/v1/content-blocks`,
        {
          title,
          contentText,
        },
  ...
  const handleBannerSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL_BO_CLIENTE}/api/v1/banners`,
        {
          title: bannerTitle,
          landingText,
          buttonText,
      ...
      if (response.status === 201) {
        console.log(response.data.banner.id, "Banner creado con éxito");
  ...
  return (
    <div className="max-w-md mx-auto p-4 bg-white shadow-md rounded">
      ...
      {success && <p className="text-green-500 mb-4">Formulario enviado con éxito!</p>}
      ...
      <form onSubmit={handleBannerSubmit}>
        ...
        <div className="mb-4">
          <label htmlFor="landingText" className="block text-sm font-medium text-gray-700">
            Texto de Bienvenida
          </label>
          <textarea
            id="landingText"
