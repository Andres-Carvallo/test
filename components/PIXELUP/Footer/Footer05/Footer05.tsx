...
export default function Footer() {
  ...
  const [configuredSocialNetworks, setConfiguredSocialNetworks] = useState<SocialNetwork[]>([]);
  const { logo } = useLogo();

  // Filtrar los enlaces del menú que son visibles
  const menuItems = mainMenuConfig.showInFooter
    ? mainMenuConfig.links.filter(
        (link) => link.isVisible && link.title !== "Colecciones" && link.title !== "Inicio"
      )
    : [];
  ...
  const enabledSocialNetworks = configuredSocialNetworks.filter(network => network.enabled && network.url);
  ...
  const hasSocial = enabledSocialNetworks.length > 0 || (socialConfig.showInFooter && socialItems.length > 0);
              ...
              {hasSocial && (
                <div className="text-center sm:text-left">
                  ...
                  <ul className="mt-4 space-y-4">
                    ...
                    {enabledSocialNetworks.length === 0 && socialItems.map((social, index) => (
                      <li
                        key={`fallback-${index}`}
                        className="flex items-center justify-center sm:justify-start"
                      >
                ...
                {enabledSocialNetworks.map((social, index) => (
                  <li key={`mobile-configured-${index}`}>
                    <a
                      ...
                      rel="noreferrer"
                      target="_blank"
                      className="text-white transition hover:opacity-35"
                    >
