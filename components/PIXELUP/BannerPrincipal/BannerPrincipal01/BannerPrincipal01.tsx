...
const BannerPrincipal01: React.FC = () => {
  ...
  useEffect(() => {
    if (bannerData && bannerData.bannerImages.length > 1) {
      const interval = setInterval(() => {
        setCurrentIndex(
          (prevIndex) => (prevIndex + 1) % bannerData.bannerImages.length
        );
  ...
  const handlePrev = () => {
    setCurrentIndex((prevIndex) =>
      prevIndex === 0
        ? (bannerData?.bannerImages.length ?? 0) - 1
        : prevIndex - 1
    );
  };
  ...
  const handleNext = () => {
    setCurrentIndex((prevIndex) =>
      prevIndex === (bannerData?.bannerImages.length ?? 0) - 1
        ? 0
        : prevIndex + 1
    );
  };
  ...
  const currentImage = bannerData.bannerImages[currentIndex];
  const multipleImages = bannerData.bannerImages.length > 1;
