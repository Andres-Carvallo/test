...
const BannerPrincipal01BO: React.FC = () => {
  ...
  // Estados para el panel de ordenamiento
  const [showOrderingPanel, setShowOrderingPanel] = useState<boolean>(false);
  ...
  const fetchBannerHome = async () => {
    ...
    try {
      ...
      const sortedImages = imagesWithDefaults.sort((a: BannerImage, b: BannerImage) => {
      ...
      const desktopImages = sortedImages.filter((image: BannerImage) => {
  ...
  // Funciones para el panel de ordenamiento
  ...
  const moveBannerDown = (index: number) => {
    if (index === bannerData.length - 1) return;
  ...
  const saveBannerOrder = async () => {
    await updateBannerOrder(bannerData);
  };

          const updateBannerOrder = async (newOrder: BannerImage[]) => {
          try {
            setIsReordering(true);
            const token = getCookie("AdminTokenAuth");
            const bannerId = `${process.env.NEXT_PUBLIC_BANNERPRINCIPAL01_ID}`;

            // Actualizar el orderNumber de cada banner
            const updatePromises = newOrder.map((banner, index) => {
              // Actualizar el JSON de landingText con el nuevo orderNumber
              let updatedLandingText = banner.landingText;
              try {
                const landingTextObj = JSON.parse(banner.landingText);
                landingTextObj.orderNumber = index + 1;
                updatedLandingText = JSON.stringify(landingTextObj);
              } catch (error) {
                console.error("Error al actualizar landingText:", error);
              }

              // Construir payload específico para actualización de orden
              // NO incluir mainImage y mobileImage para evitar errores de validación
              const updatePayload = {
                id: banner.id,
                title: banner.title,
                landingText: updatedLandingText, // Usar el landingText actualizado
                buttonLink: banner.buttonLink,
                buttonText: banner.buttonText,
                mainImageLink: banner.mainImageLink,
                orderNumber: index + 1, // Mantener el envío del orderNumber a la API
              };

              return axios.put(
                `${process.env.NEXT_PUBLIC_API_URL_BO_CLIENTE}/api/v1/banners/${bannerId}/images/${banner.id}?siteId=${process.env.NEXT_PUBLIC_API_URL_SITEID}`,
                updatePayload,
                {
                  headers: {
                    Authorization: `Bearer ${token}`,
            ...
            <button
              ...
              title={
                ...
                isAddingImage
                  ...
                  ? "No puedes ordenar mientras estás creando un banner"

                  : showOrderingPanel

                  ? "Ocultar panel de ordenamiento"

                  : "Mostrar panel de ordenamiento"

              }

            >

              {showOrderingPanel ? "Ocultar Orden" : "Ordenar Banners"}
      ...
      {showOrderingPanel && (
        <div className="bg-white rounded-lg shadow-sm border border-gray-100">
          <div className="p-4 border-b border-gray-100">
            <h3 className="text-lg font-medium text-gray-900">
              Ordenar Banners ({bannerData.length} banners)
            </h3>
            <p className="text-sm text-gray-600 mt-1">
              Arrastra y suelta los banners para reordenarlos, o usa las flechas para moverlos uno por uno.
            </p>
          </div>
          ...
          <div className="p-4">
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-4">
              <p className="text-sm text-blue-800">
                💡 <strong>Consejo:</strong> Puedes arrastrar y soltar los banners o usar las flechas ↑↓ para reordenarlos.
              </p>
            </div>
            ...
            {isReordering && (
              <div className="flex items-center justify-center py-4">
                ...
                <span className="ml-2 text-sm text-gray-600">Actualizando orden...</span>
              </div>
            )}
                ...
                {(provided) => (
                  ...
                  >
                    {bannerData.map((banner, index) => {
                      const config = parseDisplayConfig(banner.landingText);
                      const isCurrentBanner = banner.id === bannerData[currentIndex]?.id;
                            ...
                            >
                              ...
                              <div className="flex items-center space-x-2">
                                ...
                                <button
                                  ...
                                  disabled={index === bannerData.length - 1 || isReordering}
                                  className={`p-1 rounded ${
                                    index === bannerData.length - 1 || isReordering
                                      ? "text-gray-300 cursor-not-allowed"
                                      : "text-gray-600 hover:text-primary hover:bg-gray-100"
                                  }`}
                                >
