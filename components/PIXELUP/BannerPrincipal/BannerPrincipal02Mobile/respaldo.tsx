...
const BannerPrincipal02: React.FC = () => {
  ...
  const [isMobile, setIsMobile] = useState<boolean>(false); // Nuevo estado para detectar móviles

  const fetchBannerHome = async (bannerId: string) => {
    try {
      setLoading(true);
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      setIsMobile(window.innerWidth < 768); // Definimos móvil como un ancho menor a 768px
    };

    // Escuchar cambios en el tamaño de la pantalla
    window.addEventListener("resize", handleResize);
    handleResize(); // Detectar en el montaje inicial

    const bannerId = isMobile
      ? `${process.env.NEXT_PUBLIC_BANNERPRINCIPAL02MOBILE_ID}`
      : `${process.env.NEXT_PUBLIC_BANNERPRINCIPAL02_ID}`;

    // Llamar a la API con el BannerId correspondiente
    fetchBannerHome(bannerId);
  ...
  useEffect(() => {
    if (bannerData && bannerData.images.length > 1) {
      ...
      }, 8000); // Aumenta el tiempo del intervalo a 8000ms (8 segundos)
  ...
  if (!bannerData) {
    return <div className="w-full text-center p-6">No banner data available</div>;
  }
      ...
      <div className="relative font-sans before:absolute before:w-full before:h-full before:inset-0 before:bg-black before:opacity-30 before:z-10">
        <div className="absolute inset-0 w-full h-full overflow-hidden">
          {bannerData.images.map((image, index) => (
            <Link href={addHttp(image.buttonLink)} key={index} className="absolute inset-0 w-full h-full z-0">
              <img
