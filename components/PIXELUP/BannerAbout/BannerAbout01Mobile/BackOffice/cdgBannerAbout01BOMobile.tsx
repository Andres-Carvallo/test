...
const BannerAbout01BOMobile: React.FC<BannerAboutProps> = ({
  About01BOData,
}) => {
  const { BannerId, BannerImageId } = About01BOData;
  const [isMainImageUploadedMobile, setIsMainImageUploadedMobile] =
    useState(false);
  const [bannerData, setBannerData] = useState<any | null>(null);
  const [mainImageMobile, setMainImageMobile] = useState<string | null>(null);
  ...
  const [formDataMobile, setFormDataMobile] = useState<any>({
    title: "",
    landingText: "",
  ...
  const fetchBannerHome = async () => {
    try {
      ...
      const bannerId = `${process.env.NEXT_PUBLIC_BANNER_ABOUTMOBILE_ID}`;
      const bannerImageId = `${process.env.NEXT_PUBLIC_BANNER_ABOUTMOBILE_IMGID}`;
  ...
  const handleEditorChange = (value: string) => {
    if (value.length <= MAX_CHARACTERS) {
      setFormDataMobile({ ...formDataMobile, landingText: value });
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updatedDataWithoutImage = {
        ...formDataMobile,
        orderNumber: formDataMobile.orderNumber,
      };
      ...
      setLoading(false); // Ocultar el indicador de carga
      setIsMainImageUploadedMobile(false);
    }
  };
  ...
  const handleImageChange = (
    ...
    if (file) {
      ...
      reader.onload = () => {
        ...
        setFormDataMobile((prevFormDataMobile: any) => ({
          ...prevFormDataMobile,
          [imageKey]: imageInfo,
        }));
  ...
  const handleCrop = async () => {
    ...
    try {
      ...
      setMainImageMobile(base64);
      setIsModalOpen(false);
      setIsMainImageUploadedMobile(true);
    } catch (error) {
    ...
    >
      <div className="">
        {bannerData && (
          <div className="relative font-[sans-serif] before:absolute before:w-full before:h-full before:inset-0 before:bg-black before:opacity-0 before:z-10">
            <div className="flex justify-center items-center w-full h-[300px] overflow-hidden relative">
              <img
      ...
      >
        <input
          type="number"
          name="orderNumber"
          value={formDataMobile.orderNumber}
        ...
        <input
          type="text"
          name="mainImageLink"
          value={formDataMobile.mainImageLink}
          onChange={handleChange}
          className="hidden w-full px-4 py-2 mb-4 border border-gray-300 rounded-md"
        />{" "}
        <h3 className="font-normal text-primary">
        ...
        <ReactQuill
          value={formDataMobile.landingText}
        ...
        <div className="text-right text-sm text-gray-600">
          {formDataMobile.landingText.length}/{MAX_CHARACTERS}
        </div>
        {formDataMobile.landingText.length > ALERT_CHARACTERS && (
          <div
        ...
        <div>
          <input
            ...
            className="hidden"
            onChange={(e) =>
              handleImageChange(e, setMainImageMobile, "mainImageMobile")
            }
          />
          {isMainImageUploadedMobile ? (
            <div className="flex flex-col items-center mt-3 relative">
              ...
              <button
                ...
                onClick={() => handleClearImage(setMainImageMobile)}
              >
      ...
      {isModalOpen && (
        ...
        >
          <div className="relative h-96 w-full">
            <Cropper
              image={mainImageMobile || ""} // Asegurar que se pasa una cadena no nula
              crop={crop}
              zoom={zoom}
              aspect={1080 / 300}
...
export default BannerAbout01BOMobile;
