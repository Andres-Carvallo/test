...
import React, {
  ...
  function Colecciones() {
    ...
    const [mobileImageColeccion, setMobileImageColeccion] = useState<string | null>(null);
    ...
    const handleMobileCrop = async () => {
      ...
      try {
        const croppedImage = await getCroppedImg(mobileImageColeccion, croppedAreaPixels);
        if (!croppedImage) {
          console.error("Error al recortar la imagen: croppedImage es null");
          return;
        }
  ...
  {/*             <label className="block mt-4">
    ...
    {isMobileImageUploaded ? (
      ...
      <div>
        <h3 className="font-normal text-primary">Foto Móvil <span className="text-primary">*</span></h3>
        <label
        ...
        >
          <div className="flex flex-col justify-center items-center">
            ...
            <p className="mb-2 text-sm text-gray-500 dark:text-gray-400">
              <span className="font-semibold">Subir Imagen para Móvil</span>
            </p>
            <p className="text-xs text-gray-500 dark:text-gray-400">PNG, JPG o Webp (800x800px)</p>
          </div>
        </label>
      </div>
    )}
